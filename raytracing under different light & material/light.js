...
// ===YOUR CODE STARTS HERE===
		...
		let edge1 = this.from.clone().sub(shadingPoint).normalize();
		let edge2 = this.from.clone().sub(this.to).normalize();
		let cos_angle = edge1.dot(edge2);
		// the object in the spot light
		if (cos_angle > Math.cos(this.cutoff * (1/2) / 180 * Math.PI)) {
			ls.intensity = this.intensity.clone().multiplyScalar(Math.pow(cos_angle, this.exponent)).multiplyScalar(1/ls.direction.lengthSq());
		}
		else {
			ls.intensity = backgroundColor
		}
		ls.direction.normalize();
		return ls;
// ---YOUR CODE ENDS HERE---
